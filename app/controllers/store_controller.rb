...
class StoreController < ApplicationController
  ...
  def search
    ...
    @products = Product.available.order('name ASC').where(["(#{Product.connection.quote_column_name("name")} LIKE ? OR #{Product.connection.quote_column_name("code")} = ?)", "%#{@search_term}%", @search_term]).paginate(:page => params[:page],:per_page => 10)
